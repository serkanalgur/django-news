...
class Category(models.Model):
    ...
    cat_slug = models.SlugField(
        max_length=200, default="", blank=True, unique=True)

    def save(self, *args, **kwargs):
        if not self.id:
            self.cat_slug = slugify(self.cat_name, replacements=[
                                    ['Ə', 'E'], ['ə', 'e']])
...
class News(models.Model):
    ...
    author = models.ForeignKey(
        User, blank=True, on_delete=models.CASCADE, related_name='news')
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            self.slug = slugify(self.title, replacements=[
                                ['Ə', 'E'], ['ə', 'e']])
